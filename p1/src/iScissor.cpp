...
void InitNodeBuf(Node* nodes, const unsigned char* img, int imgWidth, int imgHeight)
{
    ...
    //Sad, but allocate a blurred image in both double and char types, to hold the blurred image

    double * blurred_image  = (double * ) malloc (sizeof(double)*3*imgHeight*imgWidth);
    unsigned char * blurred_image_c  = ( unsigned char * ) malloc (sizeof(char)*3*imgHeight*imgWidth);

    //Compute the blurred image
    image_filter(blurred_image, img, NULL,
                   imgWidth,  imgHeight,
                  (const double* ) &blur_kernel, 3,3,1,0);

    //Downcast the resulting blurred image so that it can be used as a char
    for (x=0;x<3*imgHeight*imgWidth;x++)
        blurred_image_c[x]=blurred_image[x];

    for (x=0;x<imgWidth;x++)
        for(y=0;y<imgHeight;y++)
            for(k=0; k<8; k++){
                pixel_filter(pixel, x, y,blurred_image_c, imgWidth, imgHeight,
                    kernels[k], 3,3,1,0);
    ...
    //Re-normalize the link weights by MaxD, and also by link length
    for (x=0;x<imgWidth;x++)
                for(y=0;y<imgHeight;y++)
                    for(k=0;k<8;k++)
                        nodes[y*imgWidth+x].linkCost[k] = (maxD - nodes[y*imgWidth+x].linkCost[k])/LENGTHS[k & 1];
    free(blurred_image);
    free(blurred_image_c);
    ...
    const unsigned char* selection, int numExpanded)
{
    ...
    //Enforce that the seed has no parent node.
    ...
    while (!pq.empty()){
        //Chose the node on the fronteir with the smallest  cost.
        q = pq.top();
