...
public:
    ...
    int GetTrueSize( void ) const {
        return m_iSize;
    }
    bool IsIndexValid( const int i ) const {
        return ( i >= 0 ) && ( i < m_iTail );
    }

    void SetTailAt( const int i ) {
        m_iTail = i;
    }
    void SetSize( int );
...
public:
    ...
    void Next( CTypedPtrDblElement *pNext ) {
        m_pNext=pNext;
    }
    CTypedPtrDblElement *Next( void ) const {
        return m_pNext;
    }

    void Prev( CTypedPtrDblElement *pPrev ) {
        m_pPrev=pPrev;
    }
    CTypedPtrDblElement *Prev( void ) const {
        return m_pPrev;
    }
};
...
public:
    ...
    void SetCircular( int c ) {
        m_iCircular = c;
    }
    int IsCircular( void ) const {
        return m_iCircular;
    }

    CTypedPtrDblElement <T> *GetHeadPtr( void ) const {
        return m_pSentinel->Next();
    }
    CTypedPtrDblElement <T> *GetTailPtr( void ) const {
        return m_pSentinel->Prev();
    }

    bool IsSentinel( const CTypedPtrDblElement <T> *pEle ) const {
        return pEle == m_pSentinel;
    }
    ...
    T *RemovePrev( CTypedPtrDblElement <T> * pEle ) {
        return Remove(pEle->Prev());
    }
    T *RemoveNext( CTypedPtrDblElement <T> * pEle ) {
        return Remove(pEle->Next());
    }

    CTypedPtrDblElement < T > * AddHead( T *t ) {
        return AddNext(m_pSentinel, t);
    }
    CTypedPtrDblElement < T > * AddTail( T *t ) {
        return AddPrev(m_pSentinel, t);
    }

    T *RemoveHead( void ) {
        return Remove(m_pSentinel->Next());
    }
    T *RemoveTail( void ) {
        return Remove(m_pSentinel->Prev());
    }
